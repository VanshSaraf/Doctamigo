...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      endTime.setHours(21, 0, 0, 0); // End time for slots is 21:00 (9 PM)

      let timeSlots = [];

      // Check if today is the current day and time is past 21:00
      if (i === 0 && today.getHours() >= 21) {
        timeSlots.push({
          datetime: currentDate,
          time: "Not Available", // No slots for today
          available: false,
        });
      } else {
        // Set starting time
        if (i === 0) {
          currentDate.setHours(Math.max(today.getHours() + 1, 10)); // Start 1 hour ahead or 10 AM
      ...
      // Add generated slots for the day
      setDocSlots((prev) => [...prev, timeSlots]);
    }
  };
